...
import UruStars from './uru-apodo.png';
import Icon from '../../../icon.png';
import Icon2 from '../../../flecha-abajo.png';
import Icon3 from '../../../cerrar-sesion.png';

const Uruguay = ({ albumId, savedStickers, pastedStickers, setPastedStickers}) => {
    ...
    const cards = Array.from({ length: 12 }, (_, index) => index + 89);
    ...
    return (
        <div className='full-screen'>
            ...
            <div className='album-container'>
                <div className='uruguay-album'>
                    <div className='title-country-container'>
                        <img src={Uru} alt="Bandera de Uruguay" className="uru-flag" />
                        <h1 className="uru-title"> URUGUAY</h1>
                    </div>
                    <img src={UruStars} alt="Estrellas de Uruguay" className="uru-stars" />
                    <div className="row1">
                        {firstRow.map(card => (
                            <div key={card} className={`card ${isStickerSaved(card) ? 'highlight' : ''}`} onClick={() => handleCardClick(card)}>                                
                                {!pastedStickersAux[card] && !pastedStickersObject[card] && (
                                    <React.Fragment>
                                        <div className='page-number'>URU</div>
                                        <div className='page-number'>{card - 53}</div>
                                        <div className='page-album'>6</div>
                                    </React.Fragment>
                                )}
