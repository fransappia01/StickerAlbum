...
import './Inglaterra.css';
import Ing from './ing.png';
import IngStars from './ing-apodo.png';
import Icon from '../../../icon.png';
import Icon2 from '../../../flecha-abajo.png';
import Icon3 from '../../../cerrar-sesion.png';

const Inglaterra = ({ albumId, savedStickers, pastedStickers, setPastedStickers}) => {
    ...
    const cards = Array.from({ length: 12 }, (_, index) => index + 113);
    ...
    return (
        <div className='full-screen'>
            ...
            <div className='album-container'>
                <div className='inglaterra-album'>
                    <div className='title-country-container'>
                        <img src={Ing} alt="Bandera de Inglaterra" className="ing-flag"/>
                        <h1 className="ing-title"> INGLATERRA </h1>
                    </div>
                    <img src={IngStars} alt="Estrellas de Inglaterra" className="ing-stars"/>
                    <div className="row1">
                        {firstRow.map(card => (
                            <div key={card} className={`card ${isStickerSaved(card) ? 'highlight' : ''}`} onClick={() => handleCardClick(card)}>                                
                                {!pastedStickersAux[card] && !pastedStickersObject[card] && (
                                    <React.Fragment>
                                        <div className='page-number'>ING</div>
                                        <div className='page-number'>{card - 53}</div>
                                        <div className='page-album'>8</div>
                                    </React.Fragment>
                                )}
